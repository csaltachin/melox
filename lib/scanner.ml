type t = { source : string; len : int; pos : int; col : int; line : int }
(** Type of scanner. Maintains current position, column and line number. *)

type scanned_t = RawToken of Token.raw_t | Whitespace | Comment | EndOfSource
type eof_error = Eof
...
let match_keyword_or_identifier =
  ...
  let find_keyword_opt =
    [|
      ...
      ("while", While);
    |]
    |> Array.to_seq |> Hashtbl.of_seq |> Hashtbl.find_opt
  in
  fun lexeme ->
...
let advance scanner =
  ...
  | Ok '\n' ->
      Ok
        { scanner with pos = scanner.pos + 1; col = 0; line = scanner.line + 1 }
...
let consume_lexeme scanner =
  ...
  | Ok '!' -> (
      ...
      | Ok '=' ->
          (scanner |> advance_ceil |> advance_ceil, Ok (RawToken BangEqual))
      | _ -> (advance_ceil scanner, Ok (RawToken Bang)))
  | Ok '=' -> (
      match peek_next scanner with
      | Ok '=' ->
          (scanner |> advance_ceil |> advance_ceil, Ok (RawToken EqualEqual))
      | _ -> (advance_ceil scanner, Ok (RawToken Equal)))
  | Ok '>' -> (
      match peek_next scanner with
      | Ok '=' ->
          (scanner |> advance_ceil |> advance_ceil, Ok (RawToken GreaterEqual))
      | _ -> (advance_ceil scanner, Ok (RawToken Greater)))
  | Ok '<' -> (
      match peek_next scanner with
      | Ok '=' ->
          (scanner |> advance_ceil |> advance_ceil, Ok (RawToken LessEqual))
